...
export async function migrate(): Promise<void> {
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_client_channels_customer ON client_channels(stripe_customer_id)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_client_channels_status ON client_channels(status)`;
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_client_channels_active_identity
    ON client_channels(stripe_customer_id, customer_email, platform) WHERE status <> 'cancelled'
  `;
  await sql`
    INSERT INTO client_channels (id, customer_email, buffer_channel_id, platform, status)
    VALUES ('agency-reference-instagram', 'internal@metroreachagency.com', '6a6156cee2638b94d7b9abf0', 'instagram', 'agency_reference'),
           ('agency-reference-facebook', 'internal@metroreachagency.com', '6a615653e2638b94d7b9aa6f', 'facebook', 'agency_reference')
    ON CONFLICT (id) DO NOTHING
  `;
  console.log("[migration] ✓ client_channels table ready");
