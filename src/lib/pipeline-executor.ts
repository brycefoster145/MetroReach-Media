...
/**
 * Build the task brief markdown for a pipeline task. Mirrors the file written
 * to the shared tasks directory so DB rows and file briefs stay identical.
 */
function buildTaskBriefMarkdown(brief: TaskBrief): string {
  return [
...
// ── Database task queue ──
// Every task brief written to the shared tasks directory is mirrored into the
// pipeline_tasks table so tasks can be auto-delegated from the database on
// every activation, without manual file-checking of /home/team/shared/tasks/.
...
/**
 * Get all pending pipeline tasks (oldest first) for auto-delegation.
 * Returns [] on any DB error so callers never crash on a DB hiccup.
 */
export async function getPendingTasks(): Promise<PipelineTask[]> {
  try {
    const rows = await sql`
      SELECT * FROM pipeline_tasks WHERE status = 'pending' ORDER BY created_at ASC
    `;
...
/**
 * Atomically claim a pending task for a delegate. Returns true only if the
 * claim succeeded (task was still pending and is now in_progress).
 */
export async function claimTask(taskId: string): Promise<boolean> {
  try {
    const result = await sql`
      UPDATE pipeline_tasks SET status = 'in_progress', updated_at = NOW()
...
/**
 * Mirror a task brief into the pipeline_tasks table after the file write has
 * succeeded. The file write is the fallback: if this DB insert fails we log it
 * and continue — the pipeline must not fail because of the queue insert.
 */
export async function insertPipelineTask(brief: TaskBrief): Promise<boolean> {
  try {
    await sql`
      INSERT INTO pipeline_tasks (
        ...
        ${brief.clientEmail},
        ${brief.company},
        ${buildTaskBriefMarkdown(brief)},
        ${brief.assignedTeam},
        ${brief.deadline}
      )
    `;
    console.log(`[pipeline-executor] Pipeline task queued in DB for client ${brief.clientId}`);
    return true;
  } catch (err: any) {
    console.error(`[pipeline-executor] Failed to insert pipeline task for client ${brief.clientId}:`, err.message);
    return false;
  }
}
/**
 ...
 */
function writeTaskBrief(brief: TaskBrief): string | null {
  const filename = `${brief.serviceSlug}-${brief.clientId}.md`;
  const path = join(TASKS_DIR, filename);

  const markdown = buildTaskBriefMarkdown(brief);
...
export async function executePipeline(client: Client): Promise<void> {
  ...
  const briefPath = writeTaskBrief(brief);
  if (!briefPath) {
    console.error(`Task brief could not be written for ${client.id} (${serviceSlug}) — marking failed`);
    await updatePipelineStatus(client.id, "failed");
    return;
  }

  // Mirror the brief into the DB task queue (non-fatal — the file brief is the fallback)
  await insertPipelineTask(brief);
