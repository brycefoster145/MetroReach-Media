...
function FreeAudit() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setStatus("success");
      if (data.redirect) {
        // Simple redirect — report page loads data from sessionStorage or Postgres.
        // No PII is ever placed in the URL.
  ...
  // NOTE: No native addEventListener("submit") — we rely on React's
  // onSubmit={handleSubmit} for JS-enabled browsers and the native
  // <form action="/api/audit/submit" method="POST"> as the JS-disabled
  // fallback. A dual-listener architecture was causing parallel fetch()
  // calls and submit failures in some browsers.
